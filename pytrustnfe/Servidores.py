...
WS_NFE_CORRECAO = 'RecepcaoEventoCarta'
...
def localizar_url(servico, estado, mod='55', ambiente=2):
    ...
    if servico in (WS_NFE_CORRECAO):
        procservico = 'RecepcaoEvento'
    else:
        procservico = servico

    if procservico in (WS_DFE_DISTRIBUICAO, WS_DOWNLOAD_NFE):
        ws = AN

    if mod in ws:
        dominio = ws[mod][ambiente]['servidor']
        complemento = ws[mod][ambiente][procservico]
    else:
        dominio = ws[ambiente]['servidor']
        complemento = ws[ambiente][procservico]

    if sigla == 'RS' and procservico == WS_NFE_CADASTRO:
        dominio = 'cad.sefazrs.rs.gov.br'
    if sigla in ('AC', 'RN', 'PB', 'SC', 'RJ') and \
       procservico == WS_NFE_CADASTRO:
...
UFPR = {
    AMBIENTE_PRODUCAO: {
        ...
        WS_NFE_CONSULTA: 'nfe/NFeConsulta4?wsdl',
